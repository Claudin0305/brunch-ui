...
const AddInscription: React.FC<Props> = ({
  data_props,
  pays,
  tranche_ages,
  civilites,
  ...
  const [showModalAffiliation, setShowModalAffiliation] =
  ...
  const [optionsAffiliation, setOptionsAffiliation] = useState<
    option[] | any
  >();
  ...
  const [formatEventOptions, setFormatEventOptions] = useState<
    option[] | any
  >();
  ...
  useEffect(() => {
    if (data_props !== null) {
      ...
      const pays = optionsPays?.filter(
        (o: any) => o.label === data_props.nomPays
      )[0];
      ...
      const dept = tableDept?.filter(
        (o: any) => o.value === data_props.ville.departementId
      )[0];
      ...
      const ville = tableVille?.filter(
        (o: any) => o.value === data_props.ville.id_ville
      )[0];
      setValue("id_ville", ville);
      setSelectedVille(ville);
      const trAge = optionsTrancheAge?.filter(
        (o: any) => o.value === data_props.idTrancheAge
      )[0];
      setValue("id_tranche_age", trAge);
      const civilite = optionsCivilite?.filter(
        (o: any) => o.value === data_props.idCivilite
      )[0];
      setValue("id_civilite", civilite);
      const aff = optionsAffiliation?.filter(
        (o: any) => o.value === data_props?.idAffiliation
      )[0];
      ...
      const mod = optionsMod.filter(
        (o: any) => o.value === data_props.mode_participation
      )[0];
      setValue("mode_participation", mod);
      setSelectedMode(mod);
      const loc = optionsLocal?.filter(
        (o: any) => o.value === data_props?.idLocal
      )[0];
      setValue("id_local", loc);
      setSelectLocal(loc);

      const mod_p = mode_paiements.filter(
        (o: any) => o.value === data_props?.modePaiement
      )[0];
  ...
  useEffect(() => {
    if (event !== null) {
      ...
      if (event?.format_event === "HYBRIDE") {
        ...
        const result = format_events.filter(
          (f) => f.value === event?.format_event
        );
          ...
          const sendMessage = () => {
            axios
              .post(
                ...
                {
                  id_event: response.data.data.idEvent,
  ...
  const onSubmit: SubmitHandler<Inputs> = (data) => {
    ...
    if (
      values.abonnement_newsletter === null ||
      values.abonnement_newsletter === undefined ||
      values.abonnement_newsletter === ""
    ) {
    ...
    if (
      values.mode_participation === null ||
      values.mode_participation === undefined ||
      values.mode_participation === ""
    ) {
    ...
    if (
      values.authorisation_liste === null ||
      values.authorisation_liste === undefined ||
      values.authorisation_liste === ""
    ) {
    ...
    if (result.length > 0) {
      ...
      const result2 = participants?.filter(
        (p: any) =>
          p.email === data.email &&
          p.idEvent === event.id_event &&
          p.prenom.toLowerCase() === data.prenom.toLowerCase() &&
          p.nom.toLowerCase() === data.nom.toLowerCase()
      );
      if (result2.length > 0) {
        Swal.fire({
          // position: 'top-end',
          icon: "error",
          title:
            "Échec d’inscription Une inscription existe déjà pour cette personne(même nom, prénom, courriel).",
          // showConfirmButton: false,
          // timer: 1500
          // buttonColor:"#000000",
          // buttons:[""]
      ...
      description: (
        <div className="flex flex-col md:grid md:grid-cols-3 gap-8">
          <div className="flex-col flex md:-mt-4 z-30">
            <label className="mb-2" htmlFor={`id_civilite`}>
              {" "}
              Civilité
              <Star />{" "}
            </label>
          ...
          <div className="block">
            ...
            {errNext?.invalid_nom && (
              <div className="block">
                {" "}
                <Error text={"La valeur du nom est incorrecte"} />{" "}
              </div>
            )}
          </div>
          <div className="block">
            ...
            {errNext?.invalid_prenom && (
              <div className="block">
                {" "}
                <Error text={"La valeur du prénom est incorrecte"} />{" "}
              </div>
            )}
          </div>
          <div className="block">
            ...
            {errNext?.tel_invalid && (
              <Error text={"Le numéro de téléphone est invalide!"} />
            )}
          </div>
          <div className="block">
            <TextField
              ...
              {...register("email", {
                ...
                pattern: {
                  value: /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/,
                  message:
                    "Le format du courriel que vous avez spécifié incorrect.  Veuillez entrer un courriel valide.",
                },
              })}
              onChange={(e) => {
                //do something
                let pattern = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
                const normalizedEmail = replaceSpecialChars(e.target.value);
                setEmail(normalizedEmail);
                setErrorEmail(!pattern.test(normalizedEmail));
                // setValue('email_confirmation', "")
                // console.log(participants)
                const result = participants?.filter(
                  (p: any) =>
                    p.email === e.target.value && p.idEvent === event.id_event
                );
            ...
            {errNext?.email_invalid && (
              <div className="block">
                {" "}
                <Error
                  text={
                    "Le format du courriel que vous avez spécifié est incorrect.  Veuillez entrer un courriel valide."
                  }
                />{" "}
              </div>
            )}
          ...
          <div className="block">
            <TextField
              ...
              onChange={(e) => {
                const normalizedEmailConfirmation = replaceSpecialChars(
                  e.target.value
                );
                setIdentiqueEmail(normalizedEmailConfirmation === email);
                register("email_confirmation").onChange(e);
              }}
            />
            {/* {responseError !== null && <Error text={responseError?.libelle}/>} */}
            {errNext?.email_conf_null && <Error text={champ} />}
            {errNext?.email_confirmation && (
              <Error
                text={
                  'Le courriel que vous avez spécifié dans le champ "Confirmation de courriel" ne correspond pas à celui que vous avez indiqué dans le champ courriel.  Veuillez entrer la même valeur dans les 2 champs.'
                }
              />
            )}
          ...
          <div className="flex-col flex md:-mt-4 z-30 ">
            <label className="mb-2" htmlFor={`id_pays`}>
              {" "}
              Pays
              <Star />{" "}
            </label>
          ...
          {selectedPays && (
            <div className="flex-col flex md:-mt-4 z-30 ">
              <label className="mb-2" htmlFor={`id_departement`}>
                {" "}
          ...
          {selectedDepartement && (
            <div className="flex-col flex md:-mt-4 z-30 ">
              <label className="mb-2" htmlFor={`id_ville`}>
                {" "}
                Ville
                <Star />{" "}
              </label>
      ...
      description: (
        <div className="flex flex-col md:grid md:grid-cols-3 gap-8">
          ...
          <div className="flex-col flex md:-mt-4 z-10">
            <label className="mb-2" htmlFor={`id_affiliation`}>
              {" "}
              Affiliation{" "}
            </label>
            <Controller
            ...
            />{" "}
            {errors?.id_affiliation && (
              <Error text={errors.id_affiliation.message} />
            )}
          </div>
          <div className="flex-col flex md:-mt-4 z-20">
            <label className="mb-2" htmlFor={`mode_participation`}>
              {" "}
              Mode participation
              <Star />{" "}
            </label>
            <Controller
              ...
              render={({ field }) => (
                <Select
                  {...field}
                  placeholder={"Choisir mode participation..."}
                  isClearable
            ...
            />{" "}
            {errors?.mode_participation && (
              <Error text={errors.mode_participation.message} />
            )}
            {errNext?.mode_participation && <Error text={champ} />}
          </div>
          {selectedMode && selectedMode.value === "PRESENTIEL" && (
            <div className="flex-col flex md:-mt-4 z-10">
              <label className="mb-2" htmlFor={`id_local`}>
                {" "}
                Local
                <Star />{" "}
              </label>
              ...
              {selectLocal && (
                <p className="mt-2">
                  <span className="font-bold">Montant participation:</span>{" "}
                  {`${selectLocal?.data?.montant_participation} ${selectLocal?.data?.codeDevise}`}
                </p>
              )}
            </div>
          )}
          ...
          <div className="col-span-3">
            ...
            /> */}
              <label htmlFor="abonnement_newsletter">
                Je souhaite recevoir les communications de GRAHN-Monde
              </label>
              <div className="flex gap-8">
                <div className="flex justify-center items-center my-2">
                  <input
                    ...
                    className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600 mr-2"
                  />
                  <label htmlFor="yes1" className="mb-1">
                    Oui
                  </label>
                </div>
                <div className="flex justify-center items-center my-2">
                  <input
                    ...
                    className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600 mr-2"
                  />
                  <label htmlFor="no1" className="mb-1">
                    Non
                  </label>
                </div>
              </div>
          ...
          <div className="flex justify-end flex-col mt-8 md:flex-row gap-8 mb-8 col-span-3">
            ...
            <Button
              ...
              className={`bg-blue-600 capitalize text-white flex items-center justify-center gap-x-2 ${
                isSubmit || selectedMode === null ? "bg-blue-400" : ""
              }`}
              variant="contained"
            >
              {isSubmit && <CircularIndeterminate />}{" "}
              <span>{data_props === null ? "S'inscrire" : "Modifier"}</span>
            </Button>
          </div>
          ,
        </div>
  ...
  const handleNext = () => {
    ...
    const errC =
      values.id_civilite === null || values.id_civilite === undefined;
    ...
    if (
      values.email_confirmation === null ||
      values.email_confirmation === undefined ||
    ...
    if (
      (values.email_confirmation.trim() !== "" &&
  ...
  return (
    <div className="container">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="center">
        ...
        <section>
          <header className="py-2 text-center md:text-left mb-4">
